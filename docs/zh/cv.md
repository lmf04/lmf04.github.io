...
* **研究背景与目标**：蛋白质 - 蛋白质相互作用（PPI）网络是理解细胞功能和疾病机制的核心。传统实验方法成本高、周期长，而基于序列或结构的计算方法存在特征单一的局限性。本研究创新性地将蛋白质互作网络建模为异质图结构，融合多模态生物数据，构建高精度预测模型，为药物靶点发现提供方法论支持。目标是1开发融合蛋白质序列、结构及功能注释的多模态图学习框架，实现端到端的 PPI 预测与药物靶点优先级排序系统，在公开基准数据集上达到 SOTA 性能，为精准医疗提供算法支撑。
* **研究进展**：构建异质图模型：整合 STRING 数据库（18 万 + 蛋白质节点）和 PDB 结构数据（20 万 + 3D 坐标），设计包含序列相似性、功能域互作、空间邻近性的多关系边类型;提出 GCN-GAT 混合架构，引入层次化注意力机制捕捉长距离依赖;开发动态负采样策略，提升正负样本分布均衡性;采用知识蒸馏技术，将教师模型（Ensemble GNN）的软标签融入训练;
* **个人贡献**：主导 PyTorch 框架下的图神经网络开发，集成 PyTorch Geometric 实现高效图运算；设计模块化代码结构，包含数据预处理、图构建、模型训练和可视化模块；实现分布式训练支持（DDP 模式），在 4×A100 GPU 集群上完成模型优化；开发自动化数据管道，处理跨源数据格式差异（XML/JSON/CSV）；构建多模态特征融合方案，通过注意力机制动态分配不同模态权重；提出自适应学习率调度器，结合余弦退火和早停机制；实现模型解释性模块，基于 GNNExplainer 生成关键边重要性热力图；开发在线推理服务，支持蛋白质 ID 输入实时返回互作预测结果。
* **研究成果**：该研究成果已形成完整的 PPI 预测解决方案，计划开展基于肺癌相关蛋白互作网络的药物重定位研究。相关代码已开源至 GitHub。
...
* **个人贡献**：技术创新：提出结合迁移学习与数据增强的训练策略，优化小样本医学影像场景下的模型泛化能力；系统构建：独立完成从需求分析、算法开发到前端交互界面设计的全流程工作；成果验证：通过临床数据与科研数据集交叉验证，确保系统在真实场景中的可靠性。
...
* **《Single-cell transcriptomic profiling identifies novel biomarkers in colorectal cancer》**
  * **期刊**：（EI 收录）
  * **作者**：XXX（独立作者），Yale University 教授（通讯作者）
  * **DOI**：XXX

* **《基于图神经网络的蛋白质相互作用预测及药物靶点》**
  * **期刊**：预印版（arXiv）
  * **作者**：XXX（独立作者），山东大学 张教授（通讯作者）
  * **DOI**：XXX

* **《基于 YOLOv8 的计算机人工智能视觉在生物医学领域的应用》**
  * **期刊**：预印版（arXiv）
  * **作者**：XXX（独立作者），山东大学 张教授（通讯作者）
  * **DOI**：XXX
