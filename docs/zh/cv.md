...
* **个人贡献**：设计 PyTorch 模型架构，实现端到端训练与评估流程；整合 STRING 数据库和 PDB 结构数据，优化特征工程。
        创新动态轨迹建模：通过Monocle3构建巨噬细胞分化轨迹，揭示CMP→单核→TAM的演化路径，鉴定GZMA、COL1A1等关键驱动基因（图5）
        建立空间互作图谱：基于10x Xenium平台解析PDAC组织空间转录组，发现巨噬细胞与癌细胞呈均匀分布模式，定位NOP53、CFB等高表达基因热点区域（图12-13）
